...
const DEFAULT_COLORS: readonly [PicocolorsColor, PicocolorsColor, PicocolorsColor] = [
	"white",
	"yellow",
	"red",
] as const;

export function format(report: FormattableReport = {}, options: FormatOptions = {}): string {
...
function formatInput(result: FormattableResult & WithInput, options: FormatOptions = {}): string[] {
...
export function formatResult(
	...
	const problems = [...errors, ...warnings].map((problem) => {
		...
		const colorName: PicocolorsColor = colors[problem.level] || ("white" as const);
	...
	const summary =
		options.verbose || hasProblems ? `${deco}   found ${el} problems, ${wl} warnings` : undefined;
